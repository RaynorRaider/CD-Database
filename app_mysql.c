...
int database_start(char *name, char *pwd)
{
    ...
    if(!mysql_real_connect(&my_connection, "localhost", name, pwd, "blpcd",
                0, NULL, 0))
    {
...
int add_cd(char *artist, char *title, char *catalogue, int *cd_id)
{
    ...
    //protect the special character in title
...
int add_tracks(struct current_tracks_st *tracks)
{
    ...
    while(tracks->track[i][0])
    {
        ...
        sprintf(is, "INSERT INTO tracks(cd_id, track_id, title) VALUES(%d, %d, '%s')",
                tracks->cd_id, i+1, es);
        res = mysql_query(&my_connection, is);
        if(res)
        {
...
int get_cd_tracks(int cd_id, struct current_tracks_st *dest)
{
    ...
    sprintf(qs, "SELECT track_id, title FROM tracks WHERE tracks.cd_id = %d\
...
int find_cds(char *search_str, struct cd_search_st *dest)
{
    ...
    sprintf(qs, "SELECT DISTINCT artist.id, cd.id FROM artist, cd WHERE artist.id = cd.artist_id \
            and (artist.name LIKE '%%%s%%' OR cd.title LIKE '%%%s%%' OR cd.catalogue LIKE '%%%s%%')",ss,ss,ss);
    res = mysql_query(&my_connection, qs);
    if(res)
    {
        fprintf(stderr, "SELECT error: %s\n", mysql_error(&my_connection));
...
int delete_cd(int cd_id)
{
    ...
    //delete tracks of this CD
    sprintf(qs, "DELETE FROM tracks WHERE cd_id = '%d'", cd_id);
    res = mysql_query(&my_connection, qs);
    if(res)
    {
...
int database_count(int (*data)[3])
{
    ...
    int res;
    char qs[250];
    char *str[3] = {"cd", "tracks", "artist"};
    int i = 0;
    if(!dbconnected) 
        return 0;
    while(i<3)
    {
        sprintf(qs, "SELECT COUNT(*) FROM %s", str[i]);
        res = mysql_query(&my_connection, qs);
        if(res)
        {
            fprintf(stderr, "SELECT error: %s\n", mysql_error(&my_connection));
...
int find_cd_with_catalogue(char *catalogue)
{
    ...
    if(!dbconnected) 
        return 0;
    sprintf(qs, "SELECT id FROM cd WHERE catalogue='%s'", catalogue);
    res = mysql_query(&my_connection, qs);
    if(res)
    {
        fprintf(stderr, "SELECT error: %s\n", mysql_error(&my_connection));
